...
ORACLE_MODEL = os.getenv("SCRY_ORACLE_MODEL", "claude-haiku-4-5")
ORACLE_MAX_TOKENS = int(os.getenv("SCRY_ORACLE_MAX_TOKENS", "600"))
...
def _api_key() -> str | None:
    ...
    try:
        ...
        m = re.search(r'^ANTHROPIC_API_KEY=("?)(.+?)\1\s*$', text, re.M)
...
def _llm(system: str, user: str) -> str | None:
    """One Anthropic messages call via plain requests. Returns None on any
    failure — callers always degrade to numbers-only."""
    key = _api_key()
    if not key:
        return None
    try:
        ...
        r = requests.post(
            ...
            json={"model": ORACLE_MODEL, "max_tokens": ORACLE_MAX_TOKENS,
                  "system": system,
...
reports in over time. You see ONLY aggregate statistics — never raw traces, never \
the agent's reasoning. Rules, absolute:
- Never issue a verdict (no allow/block/safe/unsafe). You give a reading; the \
querent decides.
- Ground every sentence in the numbers you were given. If the record is thin, \
...
unremarkable, good case. y_consistency falling means the agent's declared purpose \
is drifting away from the vow's wording.
- Tone: an oracle — measured, a little austere, zero hype. 3-6 sentences. No \
...
async def vow_reading(vow_id: str, request: Request) -> JSONResponse:
    ...
    if _rate_ok(ip):
        user = (f"The vow: \"{vow['vow']['text']}\" (agent: {vow['vow']['agent']}, "
                f"declared cadence: every {vow['vow']['cadence_hours']}h, "
                f"sandbox: {vow['sandbox']}).\n"
    ...
    return JSONResponse(content={
        ...
        "interpretation_note": (
            ...
            "It saw only the aggregate stats and the vow text — never any trace. "
            "It is guidance, not measurement, and never a verdict."
